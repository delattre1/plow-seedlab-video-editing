...
### Step 10 — Zoom by HEAD-HEIGHT RATIO, SAM3-measured, VARIED + face-centered (CEO 2026-06-20)
...
- **FACE CENTERED on every zoom.** The crop **moves toward the face** (not a plain center-of-frame crop): `cx = face.center_x − crop_w/2`, `cy = face.center_y − crop_h/2`, clamped to frame. (At R ≤ 0.70 with the face centered, the head stays fully in frame — head-safe by construction.) If the subject moves too much within a run for a fixed crop to stay centered, drop that run to full-frame (no zoom) rather than ship an off-center zoom (fluid follow is OPT-IN, Step 10b).
...
- **VERIFY (per run):** SAM3 on the actual rendered crop → head fully in frame (head_top inside, with room) and face ≈ centered (face_cx ≈ 50%); crop ≥ 1920×1080; R ≤ 0.70. Log per run: head% chosen, native crop size + FHD ✓, face-centered ✓, content reason.
