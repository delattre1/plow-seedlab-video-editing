...
- **Y — SHOT-SIZE-DEPENDENT placement by the AGGRESSIVE headroom→eyeline CURVE (CEO-picked 2026-06-21). NOT face-centered in Y, NOT a hardcoded eyes number — DERIVED per run from SAM3.** The vertical placement changes with shot size (cinematography convention): at WIDE the face is **headroom-anchored** (generous space above the crown); as the shot tightens, the **headroom shrinks and the eyeline rises toward the upper third (~33%)**; at the tight end (head 70%) the **crown kisses the top, near-zero headroom**.
  ...
  - **HARD GATE — eyeline ≤ 40% on EVERY frame, NO exceptions (CEO 2026-06-21). The gate applies to ALL moves, RESETS AND FULL-FRAME INCLUDED — there is NO untouched-original exemption.** Any crop whose **derived eyeline exceeds 40% is NOT a usable frame**: REJECT it and fall back to a level that keeps the eyeline ≤ 40%.
    ...
    - **If even the widest possible gate-passing crop cannot reach ≤ 40%** (geometry — head so large/low that no in-frame, head-safe, FHD crop satisfies it), **FLAG it — do NOT silently ship a > 40% frame.** Two failure ends: the extreme-wide (small/high head) and the close-low camera (large/low head).
...
- **VERIFY (per run):** SAM3 on the actual rendered crop → head fully in frame (crown inside, chin inside); face **X-centered** (face_cx ≈ 50%); **eyeline ≤ 40% AND near 33%** (the Y-curve gate — flag any level that cannot meet ≤ 40% as unusable, especially extreme-wide); crop ≥ 1920×1080; R ≤ 0.70. Log per run: head% chosen, native crop size + FHD ✓, X-centered ✓, **derived eyeline% + headroom% (Y-curve)**, content reason.
