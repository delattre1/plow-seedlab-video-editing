...
> **HARD RULE — A/B sync is a SOLVED problem: run the PROVEN `sync_offset.py`, NEVER invent one (CEO 2026-07-02 fold; after TWO shipped desyncs from hand-rolled sync).** Use the proven offset finder **unchanged** (the historically-proven `~/multicam_stage/sync_offset.py` on the Mac Mini — scipy `correlate` + `dynaudnorm`, snaps to the integer-frame grid). Do **NOT** improvise a sync for this problem: no ad-hoc energy-envelope cross-correlation, no hand-rolled GCC-PHAT variant, no "lip-sync by eye" AS the offset finder (eye-check is the final VERIFY step below — never the measurement). The GCC-PHAT contract that follows is **exactly what the proven `sync_offset.py` implements**; generate it from the contract **only if that proven script is genuinely absent on the host, and then generate it EXACTLY — not a variant.**
...
frames). **CAUTION (CEO 2026-07-02): the `-hwaccel … -ss {vstart} -i {vfile}` input-seek above is
KEYFRAME-SNAP-prone on long-GOP HEVC/H.264 and snaps DIFFERENTLY per codec → A/B desync. For a
GUARANTEED frame-exact A/B cut, transcode each cam to all-intra ProRes FIRST, then `-ss … -c copy`
(see the Flow-3 "NEVER `-ss`-seek long-GOP originals" rule).** Write `manifest.json` (per piece: Cam A start, n_frames, label, matched script piece).
...
- **HARD RULE 5 — NO full-frame segment, and every crop CHANGE is ≥ 10% RELATIVE in EITHER direction (CEO 2026-06-25, harden #5 — corrected).** Two requirements:
  - **(a) NO segment is the raw full-frame.** Every shipped shot/segment applies a **real, deliberate crop** — never the untouched 3840×2160 frame. A full-frame segment that **repositions** (an X slide with no zoom) is the exact bug the CEO caught. If a beat cannot satisfy all gates (eyeline ≤ 42, head ≤ 70, headroom ≥ 7, res-floor 2688×1512) with a real crop on one camera, **switch the beat to the other camera** — never ship it full-frame. (The widest "reset" framing is the widest gate-passing crop, which is itself a real crop — never the raw frame; see the reset rule.)
  - **(b) Every crop CHANGE between consecutive segments is ≥ 10% RELATIVE magnitude, in EITHER direction.** A zoom-**IN** (`new_crop_w ≤ 0.90·current_crop_w`) **OR** a zoom-**OUT** (`new_crop_w ≥ current_crop_w / 0.90`) are **BOTH valid** — the ≥10% is a relative-magnitude change, not only a punch-in (this is HARD RULE 4 applied in both directions). **Below 10% relative → HOLD: no change at all — crop size AND X position IDENTICAL to the previous segment.**
  - **REPOSITION IS NOT THE BUG — it is CORRECT and EXPECTED when it accompanies a ≥10% crop change** (the same camera held at crop1→crop2→crop3 reframes at each change). The bug is *reposition with NO crop change* (a HOLD that nonetheless slid X) **or** *at full-frame*. So: X moves ONLY together with a ≥10% crop change; a HOLD holds X too. **A "run"/segment ends at the next CROP CHANGE (≥10% in OR out) — not necessarily a camera switch; crop size AND X are set together per segment and held within it.**
...
- **Y — SHOT-SIZE-DEPENDENT placement by the AGGRESSIVE headroom→eyeline CURVE (CEO-picked 2026-06-21). NOT face-centered in Y, NOT a hardcoded eyes number — DERIVED per run from SAM3.** The vertical placement changes with shot size (cinematography convention): at WIDE the face is **headroom-anchored** (generous space above the crown); as the shot tightens, the **headroom shrinks and the eyeline rises toward the upper third (~33%)** — **bounded by `HEADROOM_FLOOR = 7%`**: the eyeline only rises as far as it can while keeping ≥ 7% headroom above the crown, so the crown never kisses the top / leaves frame on natural head movement. *(Previously the curve let the crown reach ~0% headroom at the tight end; the 7% `HEADROOM_FLOOR` now supersedes that.)*
  ...
  - **HARD GATE — eyeline ≤ 42%, evaluated (CEO 2026-06-22, relaxed from 40%), at the BEAT START, for ALL move types with NO exemption (CEO 2026-06-21). The gate applies to every move, RESETS AND FULL-FRAME INCLUDED — there is NO untouched-original exemption.** Any crop whose **beat-start eyeline exceeds 42% is NOT a usable framing**: REJECT it and fall back to a level that keeps the (beat-start) eyeline ≤ 42%.
    - **A RESET / wide / "drop to full-frame" move resolves to the WIDEST framing that still keeps eyeline ≤ 42% — NOT a raw full-frame when full-frame exceeds 42%.** Compute the widest gate-passing crop directly: `crop_h_reset = min(2160, (2160 − eyes_y) / (1 − EYELINE_GATE))` where `EYELINE_GATE = 0.42` (so the denominator is `0.58` — it is `1 − the eyeline gate`, NOT a magic number; if the gate changes, this tracks it) — the largest crop whose top-of-frame placement `cy = 2160 − crop_h` still lands the eyeline at exactly the gate (42%); `crop_w = crop_h · 16/9`, X-centered. **THE RESET IS NEVER THE RAW FULL-FRAME (HARD RULE 5a):** it is the widest **gate-passing crop** — a real, deliberate crop. Even a high-sitting / small-head wide camera whose untouched full-frame would pass the eyeline gate must ship a **real wide crop, not the raw 3840×2160 frame** (a full-frame segment that repositions is the bug); use the widest gate-passing crop (or, if that resolves to full-frame, the next wider ladder step that is still a real crop). A low-sitting cam-A reset is a slightly-tighter wide crop as before. (No absolute "≥10%-from-full-frame" floor — that was too narrow; the constraint is simply *never the raw frame*, and crop CHANGES are ≥10% relative per HARD RULE 5b.)
...
- **RESET-to-wide** — instant jump back to the **widest gate-passing level** (use on a **new question / topic / the outro**). NOTE: the reset is **still subject to the eyeline ≤ 42% gate AND to HARD RULE 5a (NEVER the raw full-frame)** — it is the widest framing that keeps eyeline ≤ 42% and is a **real crop**, never the untouched 3840×2160 frame. On any camera a reset is a wide CROP, never the raw full frame.
...
## Flow-3 CUT_MODE — clean-disfluency (default) vs EDITORIAL content-cut (C4885 fold)
**Step-0 interview adds two Flow-3 inputs:**
- **`CUT_MODE`** ∈ {`clean-disfluency` (DEFAULT = the current behavior — remove silence + false starts, keep EVERY sentence), `editorial`}. Only `editorial` changes the cut; everything else in Flow-3 (sync, transcription-as-source-of-truth, framing, render) is identical.
- **`TARGET_MIN`** — target output length in minutes (editorial only; "keep only the most important content, ≈ N min"). Absent ⇒ no length target, select on importance alone.

`clean-disfluency` is unchanged and REMAINS the default — do NOT alter pipeline-stage-4's locked `pad 0.0/0.3`, break `0.35` for that mode.

### EDITORIAL CONTENT-CUT (`CUT_MODE=editorial`) — aggressive, transcript-driven
INTENT: the conservative pass keeps every sentence; editorial mode additionally throws out whole low-value CONTENT so a long take becomes a tight, watchable cut. It is CONTENT selection driven by the Cam-A transcript, layered ON TOP of (not replacing) the mechanical disfluency clean.
- **BLOCK selection (the aggression lives HERE, at the topic level).** The agent reads the FULL transcript and selects the core content BLOCKS to keep. THROW OUT: small-talk / pleasantries, tangents / digressions, logistics minutiae, and — within kept blocks — repeated points, abandoned-then-restated self-corrections, and rambling. Keep the distinctive substance of each retained topic. This is a reasoning step over the transcript (the landscape analog of FLOW-2 Step-4 "FIND THE GOLD"), NOT a script.
- **Mechanical clean layered ON TOP (all transcript-driven, within kept blocks):** hesitations; cut-off / self-correction fragments (trailing `--` / short dangling `-`); function-word stutters; language-specific filler tics; **PHRASE-REPEAT dedup** — an adjacent repeated 2–3-word phrase collapses to ONE occurrence, but ONLY when the repeated phrase is meaningful (**total length ≥ 6 chars**) so tiny function-word repeats don't over-trigger; and a **dead-air cap**.
- **WORD-BOUNDARY TIGHT EXCISE — the contract that makes word-cuts actually LAND (do NOT skip).** When a dropped word sits BETWEEN two kept words, hard-cut exactly at the kept-word boundaries with a tiny expand pad (**`EXPAD ≈ 0.015 s`**): keep-end = `prev.end + EXPAD`, next-keep-start = `next.start − EXPAD`. **Do NOT bridge across the dropped word with a wide trough-snap** — a wide audio-minimum search (≈ 90 ms `quiet_near`) re-enters short 60–100 ms words and the cut silently does NOT land. Trough-snapping (snap a cut to the nearest audio-envelope minimum) is for GENUINE dead-air pauses ONLY, never for a word excised between two kept words. If tight-excise is skipped, most word-cuts silently fail — which is exactly why the editorial Verify re-transcribes the OUTPUT to prove landing (see Flow-3 Verify).
- **TARGET-LENGTH loop.** Build → measure kept minutes → widen/tighten the KEEP blocks and relax/tighten the dead-air cap until output ≈ `TARGET_MIN`. Dead-air is content-dependent: reflective/contemplative content wants more breath (looser cap — `GAP_TRIM_OVER ≈ 0.90`, `GAP_TO ≈ 0.55`), punchy content tighter. Reach length by moving BLOCK boundaries first, breath second — never by touching per-word pad off its locked values.
- **LANGUAGE rule (fold from C4866/C4885) — never apply EN filler lists to another language.** In PT, `um` is the article "a/one" (NOT the English "um" filler) and `é` is the verb "is" — cutting them as fillers deletes real words. Cut PT filler tics only (`é…` hesitation, `né`, `tipo`, `então`, `hã/hum/aham`), each CONTEXT-GUARDED (e.g. `tipo` is filler only when not bracketed by the words that make it "type of"; `né` only when it abuts the next word < 0.40 s). Word-level ambiguity stays IN — aggression lives in BLOCK selection, not risky per-word deletion. Detect the transcript language and select the matching filler set; never cross-apply.

### CUT-PLAN — post the plan BEFORE the expensive render (editorial only)
Between transcript and render, the agent POSTS a short **cut-plan** for cheap course-correction, so a wrong N-min cut isn't 4K-rendered first: the KEEP blocks (topic + source time-range + rough kept-duration) and what is thrown out. Landscape analog of FLOW-2 Step-4 "FIND THE GOLD" plus an explicit pre-render gate. **The gate is INTERRUPTIBLE** — the CEO may say GO and waive it; the CONTRACT is "post the plan and give the CEO the chance to redirect before the render," NOT "block on approval."
...
## Flow-3 framing-pass robustness — SAM3 endpoint, resumability, head-safe HOLD (C4885 fold)
The framing pass (pipeline stage 5) runs many SAM3 measurements; these contracts keep it from stalling or shipping unsafe/jittery crops.
- **SAM3 ENDPOINT — avoid the contended default.** The unpinned SAM3 `/segment` on `:8100` lands on GPU0 alongside the active Parakeet STT (~8 GB) and serves ONE request at a time, so latency spikes (measured 0.4 → 5.8 s) and STALLS the framing pass. **Prefer the isolated replica `:8103` (pinned GPU1) when GPU0 is contended** — measure SAM3 latency and switch to `:8103` if slow. (The seed already notes `:8103` exists; this adds the failure-mode + the switch heuristic.)
- **Framing pass MUST be RESUMABLE** (the seed already mandates this for RENDER — extend the same discipline to framing). Cache each run's SAM3 anchors to disk and SKIP cached entries on restart; use a short SAM3 client timeout (**≤ 20 s, ≤ 2 retries**) so a GPU hiccup never blocks worker threads or loses completed work.
- **HELD-CROP HEAD-SAFETY GUARD.** A crop HELD across beats (Step-10 HARD RULE 5 X-hold) MUST be re-validated head-safe against the CURRENT beat's SAM3 head: crown AND chin still in frame, headroom not collapsed. If the subject moved and the held crop would clip the head, **BREAK the hold to the widest gate-passing head-safe crop — head-safety SUPERSEDES the aesthetic hold.** (This head-safety re-validation is the hard floor; the ideal-framing 7% headroom / 36% eyeline targets still govern fresh crops.)
- **JITTER-ELIMINATION post-pass.** After crop-chaining, snap any residual consecutive SAME-camera pair whose relative crop change is `0 < Δ < 10%` to a HOLD (a stable per-cluster crop size), so no sub-10% jitter ever ships — the enforcement companion to the MINIMUM CROP-CHANGE Verify gate.

## Flow-3 render rules (the proven ffmpeg discipline)
- **NEVER `-ss`-seek long-GOP originals for a frame-exact cut — TRANSCODE to all-intra ProRes FIRST (CEO 2026-07-02 fold; the C4916/C4806 A/B-desync root cause).** `-ss` + `-c copy` is **frame-exact ONLY on all-intra sources** (ProRes / DNxHR / MJPEG — every frame a keyframe). On **long-GOP HEVC/H.264** an input-seek (`-ss` BEFORE `-i`, ESPECIALLY with `-hwaccel`) **snaps to the nearest keyframe**, and the snap distance **differs per codec** (HEVC and H.264 GOP boundaries don't align) → the two cameras land on **DIFFERENT frames** → **A/B lip-desync**. So for any frame-exact A/B work: **transcode each source to ProRes ONCE** (`prores_videotoolbox`, all-intra), THEN the proven `-ss <t> -i seg.mov -t <d> -c copy` extract is frame-exact AND fast. Do **NOT** `-ss`-cut the raw HEVC/H.264 originals. **(Corrects the earlier implication that `-c copy`/`-ss` cutting is frame-exact on any source — it is NOT: qualify every such claim to ALL-INTRA sources only.)**
- **MIXED-CAM CONCAT NEEDS UNIFORM SAR — crop to EXACT 16:9 (SAR 1:1), or add `setsar=1` (CEO 2026-07-02 fold; the C4916 concat-fail root cause).** `concat` demands **identical Sample Aspect Ratio** across inputs; per-window crops from different cameras/crops can carry different SAR (saw `4575:4576` vs `4545:4544`) → concat dies with **"Could not open encoder before EOF."** Fix **EITHER:** make every crop **exactly 16:9** so SAR stays `1:1` (crop_h a multiple of 18, `crop_w = crop_h·16/9` — integer, square pixels), **OR** add **`setsar=1`** after each `crop` in the filtergraph. **`multicam_render_v2.py` currently lacks `setsar=1`,** so until it is patched, satisfy this via **exact-16:9 crops in the EDL** (do NOT hand-edit the proven render script mid-job).
- **RENDER_HOST ∈ {`mac-mini` (videotoolbox) — PRIMARY/DEFAULT, `server` (nvenc) — FALLBACK-ONLY} (C4885 fold; CEO correction 2026-07-01 — SUPERSEDES the earlier "default `server`" wording).** The render target is a **dedicated Mac Mini** (Apple `videotoolbox` HW ProRes+HEVC), **NOT** the server GPU by default: the Apple media engine is FASTER than the server's NVIDIA nvenc for this HW-encode workload AND it keeps renders off the CEO's primary Mac. **NEVER render on the CEO's PRIMARY working Mac** — its media engine is SHARED with OBS/streaming, so a HW render QUEUES behind OBS and a mux CRAWLS (~35 min for a 31-min edit); "off the working Mac" is MEDIA-ENGINE contention, not CPU (the encode is HW, ~1 core). **PRIMARY = the Mac Mini** (`delattres-mac-mini`, Apple M4 — the historically-proven multicam render box `mac-mini:~/multicam`). **FALLBACK = server nvenc, ONLY when the Mac Mini is UNAVAILABLE** (offline / busy / disk-full): the Linux/CUDA nvenc recipe below (`hevc_nvenc -rc vbr -cq 19`, concat-FILTER not ProRes, 4:2:2-10bit sw-decode, concat-FILTER-final-not-`-c copy` to avoid green-frame joins; ref impl `render_nvenc.py`) stays as the documented fallback. (Distinct from the LOCAL-ONLY disk rule below — either node renders to ITS OWN local disk = compliant.)
- **Mac Mini render-node DISPATCH — the PRIMARY path (a contract to derive the script from, NOT to paste).** **REACH:** the Mac Mini is a tailnet node reached over SSH (alias `mac-mini`); verify it is online AND has ffmpeg-with-videotoolbox before staging. **GOTCHA: a non-interactive SSH shell has NO Homebrew on PATH → invoke ffmpeg by its ABSOLUTE path `/opt/homebrew/bin/ffmpeg`** (a bare `ffmpeg` "not found" over SSH is a PATH issue, not a missing binary). **STAGE:** `scp` the source video + the EDL (kept-spans) + the framing JSON + the render script to the node's LOCAL scratch — footage must be LOCAL to the render host (never a network mount); macOS rsync 2.6.9 lacks `--info=progress2`, so use `scp`. **RENDER (videotoolbox — the SAME contract as the Mac primary path, just remote):** per-segment `-hwaccel videotoolbox` decode + per-run SAM3 crop + `scale=3840:2160` → **ProRes LT intermediates** (`prores_videotoolbox -profile:v 1`; resumable/skip-existing; every pass return-code + `ffprobe`-validated; 3-digit zero-padded NUMERIC order) → concat-DEMUXER → **FINAL HW-encode** (`hevc_videotoolbox` 10-bit `p010le -tag:v hvc1`, or `h264_videotoolbox` per delivery) → PTS realign (the concat-DEMUXER path DOES need the `-itsoffset` realign) → A/V lock + no black first frame; the PRE-RENDER LOCAL-disk space gate applies on the node. **RETURN + CLEANUP:** `scp` the final master back, then DELETE the node's scratch (ProRes intermediates + source copy + frames) so it's clean for the next job. Node facts (verified 2026-07-01): `delattres-mac-mini`, Apple M4 (`Mac16,10`), `/opt/homebrew/bin/ffmpeg` (h264/hevc/prores_videotoolbox), python3, ~83 GB free.
...
- **EDITORIAL mode — CONTENT-METRICS proof (`CUT_MODE=editorial`; re-transcribe OUTPUT vs SOURCE).** Beyond the completeness + disfluency checks above, prove the editorial cut did its job with a BEFORE/AFTER measured from re-transcribing the OUTPUT against the SOURCE: **words −X%**; source→output counts for **hesitations / function-word stutters / self-corrections / phrase-repeats**; output duration vs **`TARGET_MIN`**; AND a **core-content-marker check** — the distinctive phrase of EACH kept topic is present in the output (proves the CORE survived, not merely that errors were removed). Match on semantic markers tolerating ASR normalization (same rule as the completeness check), never byte-equality with the EDL text. Reference orders of magnitude (C4885): 5221 → 2408 words (−54%); hesitations 23 → 1; function-word stutters 46 → 1; self-corrections 34 → 5; phrase-repeats 15 → 3; all 13 core topics present.
