...
- **Y — SHOT-SIZE-DEPENDENT placement by the AGGRESSIVE headroom→eyeline CURVE (CEO-picked 2026-06-21). NOT face-centered in Y, NOT a hardcoded eyes number — DERIVED per run from SAM3.** The vertical placement changes with shot size (cinematography convention): at WIDE the face is **headroom-anchored** (generous space above the crown); as the shot tightens, the **headroom shrinks and the eyeline rises toward the upper third (~33%)** — **bounded by `HEADROOM_FLOOR = 7%`**: the eyeline only rises as far as it can while keeping ≥ 7% headroom above the crown, so the crown never kisses the top / leaves frame on natural head movement. *(Previously the curve let the crown reach ~0% headroom at the tight end; the 7% `HEADROOM_FLOOR` now supersedes that.)*
  ...
  - **ENFORCE THE HEADROOM FLOOR (precedence over eyeline) (CEO 2026-06-21):** the valid `cy` band is `eyes_y − 0.42·crop_h ≤ cy ≤ crown − 0.07·crop_h` (lower bound = eyeline ≤ 42%; upper bound = headroom ≥ 7%). Place `cy` at the curve/33%-target value **clamped into this band** (and into head-safe + frame). The 7% headroom wins over the 33% eyeline goal — at tighter heads the eyeline lands lower than 33% to preserve headroom. **Joint feasibility:** eyeline ≤ 42% AND headroom ≥ 7% both hold up to head ≈ **0.73** (since `eyes_y − crown = 0.48·R·crop_h` must be ≤ `0.35·crop_h`); since 0.73 > `MAX_HEAD = 0.70`, the **70% head cap is now the binding limit** across the whole 0.40–0.70 range — the eyeline/headroom pair no longer self-limits head below the cap. If a band is ever empty, go wider.
  ...
  - **HARD GATE — eyeline ≤ 42%, evaluated (CEO 2026-06-22, relaxed from 40%), at the BEAT START, for ALL move types with NO exemption (CEO 2026-06-21). The gate applies to every move, RESETS AND FULL-FRAME INCLUDED — there is NO untouched-original exemption.** Any crop whose **beat-start eyeline exceeds 42% is NOT a usable framing**: REJECT it and fall back to a level that keeps the (beat-start) eyeline ≤ 42%.
    ...
    - **EVALUATE AT 3 SAMPLE POINTS — START / MIDDLE / END of the segment (CEO 2026-06-22; replaces the single beat-start sample).** One frame misreads where the head is (the subject moves during a beat), so SAM3 the head box at **3 points**: `start + 0.15s`, the **midpoint**, and `end − 0.15s`. **COMBINE so the static crop is valid for the WHOLE segment, not just its opening frame:** size the crop from the **median `head_h`** of the 3 samples (stable — no per-frame size jitter); then the crop's Y must satisfy the gates at ALL 3 samples — the valid `cy` band is the **INTERSECTION** of the three per-frame bands: `cy ∈ [ max_i( eyes_i − 0.42·crop_h, chin_i − crop_h, 0 ),  min_i( crown_i − 0.07·crop_h, crown_i, 2160 − crop_h ) ]` (i.e. eyeline ≤ 42 AND headroom ≥ 7 AND head-in-frame at start AND mid AND end). Place `cy` at the 33%-target (median eyes) **clamped into that intersection**; X-center on the median `face_cx`. If the intersection is empty at the chosen head level, go wider (or switch camera per the gate fallback). This keeps the head in frame and the eyeline/headroom valid across the whole segment instead of only at the opening frame. **Cost: 3× SAM3 per beat per camera — accepted.** This is NOT dense per-frame sampling — just 3 anchors; mid-take drift BETWEEN the 3 anchors is still fine. (The reset-applies-to-all-moves rule, commit `daf88c6`, still holds.)
...
- **VALID-CROP RULE SET (a crop must satisfy ALL, CEO 2026-06-21):** `eyeline ≤ 42%` AND `head ≤ MAX_HEAD (0.70)` AND `headroom ≥ HEADROOM_FLOOR (7%)` AND `crop ≥ 2496×1404` (1.3×FHD) AND `X-centered (face_cx ≈ 50%)`. If a level can't satisfy all, go wider (or switch camera); never ship a crop that violates any.
- **VERIFY (per run, at the BEAT START):** SAM3 on the actual rendered crop at START / MID / END → head fully in frame (crown inside, chin inside); face **X-centered** (face_cx ≈ 50%); **beat-start eyeline ≤ 42% AND near 33%**; **headroom ≥ 7%** (crown-to-top gap); crop ≥ 2496×1404 (1.3×FHD); R ≤ MAX_HEAD (0.70). (Flag any beat whose start cannot meet the rule set; do NOT re-check or correct mid-take drift.) Log per run: head% chosen, native crop size + FHD ✓, X-centered ✓, **derived eyeline% + headroom% (Y-curve)**, content reason.
