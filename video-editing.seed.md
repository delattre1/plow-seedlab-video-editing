...
**Zoom is defined by HEAD-HEIGHT RATIO = vertical head height (crown→chin) ÷ screen height** — NOT a crop-factor (z25/z50). SAM3 gives the face bbox; `head_height ≈ 1.25 × face_bbox.height` (crown ≈ `face.y1 − 0.25·face.height`). For a target ratio R, `crop_h = head_h / R`, `crop_w = crop_h · 16/9` (landscape), from the **full original 4K** (per the source-resolution discipline) → scale to delivery at export.
> **VERTICAL-FRAMING PARAMETERS — CEO DECISION PENDING (set from comparison HTML, NOT an invented number) (CEO 2026-06-21).** The two vertical-framing height values below are **`MAX_HEAD`** (the head-height cap) and **`HEADROOM_FLOOR`** (the minimum crown-to-top-of-frame gap). **Both are TBD — the CEO will CHOOSE them visually from a side-by-side comparison HTML of real frames, not from a number we picked.** Do **NOT** bake a value. Rationale: the prior `0.70` cap and the captured `~6–10%` headroom were never visually validated — they are placeholders only. Until the CEO picks: treat `MAX_HEAD` and `HEADROOM_FLOOR` as unset; do not ship a production edit that hardcodes either. (`HEADROOM_FLOOR` will SUBORDINATE the eyeline target — eyeline goes as high as possible without violating the floor — per the captured headroom-floor note; that precedence is also part of the pending CEO decision.)
- **HARD RULE 1 — MAX head-height cap = `MAX_HEAD` (CEO-DECISION, TBD — chosen from the comparison HTML).** Never tighter than `R = MAX_HEAD` (head fills ≤ `MAX_HEAD` of screen height). *(Was a hardcoded 0.70; UNFOLDED 2026-06-21 — the cap is a CEO-chosen parameter, not an arbitrary fixed %.)*
- **HARD RULE 2 — FHD FLOOR.** Every crop keeps **≥ 1920×1080 NATIVE** pixels — never below Full HD (`crop_w ≥ 1920` AND `crop_h ≥ 1080`). This also caps R (`R ≤ head_h/1080`). Drop/never-use any level below FHD.
- **VARIED, content-driven levels (NOT max-on-both).** Per camera run, choose R **by what is being said** (agent-reasoned, same spirit as the camera choice), within `[natural-wide, MAX_HEAD]` where natural-wide = `head_h/2160` (the full-frame ratio — can't go wider than the source) and `MAX_HEAD` is the CEO-chosen cap (TBD, above). Wider (smaller head) for setup / new questions / lists / transitions / outro; tighter (toward `MAX_HEAD`) for emphasis / key claims / punchlines / personal beats. The CLOSE cam sits near its natural, pushing up only on the strongest beats; the WIDE cam carries the big range. **Never put both cameras at max** — vary.
- **X — FACE CENTERED horizontally on every zoom.** The crop **moves toward the face** in X: `cx = face.center_x − crop_w/2`, clamped to frame. If the subject moves too much within a run for a fixed crop-x to stay centered, drop that run to the **widest gate-passing crop** (the reset framing below — still subject to the eyeline ≤ 40% gate, NOT a raw full-frame) rather than ship an off-center zoom (fluid follow is OPT-IN, Step 10b).
- **Y — SHOT-SIZE-DEPENDENT placement by the AGGRESSIVE headroom→eyeline CURVE (CEO-picked 2026-06-21). NOT face-centered in Y, NOT a hardcoded eyes number — DERIVED per run from SAM3.** The vertical placement changes with shot size (cinematography convention): at WIDE the face is **headroom-anchored** (generous space above the crown); as the shot tightens, the **headroom shrinks and the eyeline rises toward the upper third (~33%)** — **bounded by `HEADROOM_FLOOR`** (CEO-decision, TBD): the eyeline only rises as far as the headroom floor allows, so the crown never kisses the top / leaves frame on natural head movement. *(Previously the curve let the crown reach ~0% headroom at the tight end; the `HEADROOM_FLOOR` parameter — pending CEO choice from the comparison HTML — supersedes that.)*
  - SAM3 anchors: `eyes_y ≈ face.y1 + 0.35·face.height` (eyeline), `crown ≈ face.y1 − 0.25·face.height`, `chin = face.y2`.
  - **The CURVE (AGGRESSIVE, k = 0.5):** `t = clamp((R − 0.40) / (0.70 − 0.40), 0, 1)`; `headroom_frac = H_WIDE · (1 − t^0.5)` with **`H_WIDE = 0.20`** (fraction of crop height; this is the approved eyeline-shape curve — eyes ~32–40% across head 40→`MAX_HEAD`). *(The `0.70` here is the curve's SHAPE asymptote constant, NOT the head cap — the head cap is `MAX_HEAD` (CEO-TBD). The empirical ~32–40% eyes range was measured with the old 0.70 cap and is to be re-validated for the chosen `MAX_HEAD` + `HEADROOM_FLOOR` in the comparison HTML; the headroom floor will bound how high the eyeline rises.)* Then `cy = round(crown − headroom_frac · crop_h)`, **clamped** to `[0, 2160 − crop_h]` and head-safe (`cy ≤ crown` so the crown stays in frame; `cy + crop_h ≥ chin` so the chin stays in frame). The resulting eyeline is `eyes_line% = (eyes_y − cy) / crop_h`. (For R below 0.40 — wider than head 40%, e.g. the natural-wide level — `t` clamps to 0 → full `H_WIDE` headroom plateau.)
  - **GOAL:** eyeline as close to **33% (upper third)** as possible.
  - **HARD GATE — eyeline ≤ 40%, evaluated at the BEAT START, for ALL move types with NO exemption (CEO 2026-06-21). The gate applies to every move, RESETS AND FULL-FRAME INCLUDED — there is NO untouched-original exemption.** Any crop whose **beat-start eyeline exceeds 40% is NOT a usable framing**: REJECT it and fall back to a level that keeps the (beat-start) eyeline ≤ 40%.
    - **A RESET / wide / "drop to full-frame" move resolves to the WIDEST framing that still keeps eyeline ≤ 40% — NOT a raw full-frame when full-frame exceeds 40%.** Compute the widest gate-passing crop directly: `crop_h_reset = min(2160, (2160 − eyes_y) / 0.60)` (the largest crop whose top-of-frame placement `cy = 2160 − crop_h` still lands the eyeline at exactly 40%); `crop_w = crop_h · 16/9`, X-centered. If full-frame already passes (`eyes_y / 2160 ≤ 0.40` — e.g. a high-sitting / small-head wide camera) then the reset stays full-frame; otherwise (e.g. a **cam-A that sits low**, full-frame eyeline ~46–51%) the reset becomes the slightly-tighter widest gate-passing crop (~head 55–60%) — this is correct per the rule, not a defect.
    - **If even the widest possible gate-passing crop cannot reach ≤ 40%** (geometry — head so large/low that no in-frame, head-safe, FHD crop satisfies it at the beat start), **FLAG it — do NOT silently ship a > 40% beat.** Two failure ends: the extreme-wide (small/high head) and the close-low camera (large/low head). When a close-cam beat-start sits below the line even at MAX_HEAD, switching that beat's video to the wider camera (audio stays Cam A) is a valid fallback.
    - **EVALUATE THE GATE AT THE BEAT START, NOT PER-FRAME (CEO 2026-06-21 simplification).** Place the crop from **ONE representative frame at the run/beat start** (e.g. SAM3 at `start + ~0.15s`), apply the curve + gate to that single sample, and HOLD the static crop for the beat. **Natural mid-take movement — the subject leaning in / head dipping so the eyeline drifts during the beat — is ACCEPTABLE and must NOT be corrected.** Do **NOT** densely sample many frames per beat, and do **NOT** add a safety margin to chase an every-frame ≤ 40 guarantee (that over-tightens and force-switches cameras for transient dips). This complements the **stable per-run head reference** (size from the run/beat-start head, no per-frame jitter) — both place from a single stable sample and let the take breathe. (The reset-applies-to-all-moves rule above, commit `daf88c6`, still holds — it just means the *beat-start* reset is a gate-passing crop, never raw full-frame.)
...
- **VERIFY (per run, at the BEAT START):** SAM3 on the actual rendered crop **at the beat start** → head fully in frame (crown inside, chin inside); face **X-centered** (face_cx ≈ 50%); **beat-start eyeline ≤ 40% AND near 33%** (the Y-curve gate — flag any beat whose start cannot meet ≤ 40% as unusable, especially extreme-wide / close-low; do NOT re-check or correct mid-take drift); crop ≥ 1920×1080; R ≤ MAX_HEAD. Log per run: head% chosen, native crop size + FHD ✓, X-centered ✓, **derived eyeline% + headroom% (Y-curve)**, content reason.
...
- **RESET-to-wide** — instant jump back to the **widest gate-passing level** (use on a **new question / topic / the outro**). NOTE: the reset is **still subject to the eyeline ≤ 40% gate** — it is the widest framing that keeps eyeline ≤ 40%, NOT a raw untouched full-frame (see the HARD GATE above). On a low-sitting cam-A this means a reset is a slightly-tighter wide crop, not full-frame.
- **Agent-reasoned CAMERA SWITCHING** (Step 9) — the other instant change.
- **Steps land ON the silence-cut beats** (change-on-cut → reads intentional). Within a run, change the static level only at a piece boundary (a trimmed-pause beat); never ramp mid-shot. Map: ladder-up → punchline/admission; reset-wide → new question/outro (widest gate-passing crop, eyeline ≤ 40%); step-in → new answer/intensity; static → short beats; widest-gate-passing-crop → if the subject moved too much to keep a tight crop centered (NOT raw full-frame — the eyeline ≤ 40% gate applies to every move).
