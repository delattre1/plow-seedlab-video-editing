...
- **Y — SHOT-SIZE-DEPENDENT placement by the AGGRESSIVE headroom→eyeline CURVE (CEO-picked 2026-06-21). NOT face-centered in Y, NOT a hardcoded eyes number — DERIVED per run from SAM3.** The vertical placement changes with shot size (cinematography convention): at WIDE the face is **headroom-anchored** (generous space above the crown); as the shot tightens, the **headroom shrinks and the eyeline rises toward the upper third (~33%)**; at the tight end (head 70%) the **crown kisses the top, near-zero headroom**.
  ...
  - **HARD GATE — eyeline ≤ 40%.** Any crop whose **derived eyeline exceeds 40% is NOT a usable frame**: REJECT that level and fall back to a level that keeps the eyeline ≤ 40% (a wider/different usable level), or **full-frame (untouched original) if none qualifies** — full-frame is always the available reset baseline. The risk case is the **extreme-wide end where the head is small/high in frame**: flag and mark unusable any zoom level that cannot meet eyeline ≤ 40%.
...
- **RESET-to-wide** — instant jump back to the wide/natural level (use on a **new question / topic / the outro**).
- **Agent-reasoned CAMERA SWITCHING** (Step 9) — the other instant change.
- **Steps land ON the silence-cut beats** (change-on-cut → reads intentional). Within a run, change the static level only at a piece boundary (a trimmed-pause beat); never ramp mid-shot. Map: ladder-up → punchline/admission; reset-wide → new question/outro; step-in → new answer/intensity; static → short beats; full-frame → if the subject moved too much to keep a tight crop centered.
