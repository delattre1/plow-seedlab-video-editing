...
- **HARD RULE 1 — MAX = 60% head height (CEO 2026-06-21, reduced from 70%).** Never tighter than R = 0.60 (head fills ≤60% of screen height). NOTE: a close camera whose **natural** (full-frame) head already exceeds 0.60 cannot satisfy this cap at any crop (you can't zoom out past full-frame) → that beat must use the WIDER camera. (This change only lowers the max-head cap; the Y-curve eyeline formula and the 40% eyeline gate are unchanged.)
...
- **VARIED, content-driven levels (NOT max-on-both).** Per camera run, choose R **by what is being said** (agent-reasoned, same spirit as the camera choice), within `[natural-wide, 0.60]` where natural-wide = `head_h/2160` (the full-frame ratio — can't go wider than the source). Wider (smaller head) for setup / new questions / lists / transitions / outro; tighter (toward the 0.60 cap) for emphasis / key claims / punchlines / personal beats. The CLOSE cam sits near its natural; **if that natural already exceeds the 0.60 cap (a tight close camera), the beat uses the WIDE cam instead** (you cannot zoom out past full-frame). The WIDE cam carries the big range. **Never put both cameras at max** — vary.
...
- **Y — SHOT-SIZE-DEPENDENT placement by the AGGRESSIVE headroom→eyeline CURVE (CEO-picked 2026-06-21). NOT face-centered in Y, NOT a hardcoded eyes number — DERIVED per run from SAM3.** The vertical placement changes with shot size (cinematography convention): at WIDE the face is **headroom-anchored** (generous space above the crown); as the shot tightens, the **headroom shrinks and the eyeline rises toward the upper third (~33%)**; at the tight end (the **60% head cap**) the headroom is at its minimum.
  ...
  - **HARD GATE — eyeline ≤ 40%, evaluated at the BEAT START, for ALL move types with NO exemption (CEO 2026-06-21). The gate applies to every move, RESETS AND FULL-FRAME INCLUDED — there is NO untouched-original exemption.** Any crop whose **beat-start eyeline exceeds 40% is NOT a usable framing**: REJECT it and fall back to a level that keeps the (beat-start) eyeline ≤ 40%.
    ...
    - **If even the widest possible gate-passing crop cannot reach ≤ 40%** (geometry — head so large/low that no in-frame, head-safe, FHD crop satisfies it at the beat start), **FLAG it — do NOT silently ship a > 40% beat.** Two failure ends: the extreme-wide (small/high head) and the close-low camera (large/low head). When a close-cam beat-start sits below the line even at head 60% (the cap), switching that beat's video to the wider camera (audio stays Cam A) is a valid fallback.
