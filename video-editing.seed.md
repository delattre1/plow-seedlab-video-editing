...
- **HARD RULE 2 — RESOLUTION FLOOR = 1.3×FHD = 2496×1404 (CEO-decided 2026-06-21; was 1920×1080, plain FHD looked soft).** Every crop region in the 4K source keeps **≥ 2496×1404 NATIVE** pixels (`crop_w ≥ 2496` AND `crop_h ≥ 1404`). This caps the max zoom (`R ≤ head_h/1404`): a crop that would fall below 2496×1404 is REJECTED — the shot stays wider but sharp. (Effect: a small-head WIDE camera can only punch in until its crop hits 1404 tall — e.g. head_h≈700 ⇒ max head ≈ 0.50 — so the wide cam never upscales a tiny soft region.)
...
- **VARIED, content-driven levels (NOT max-on-both).** Per camera run, choose R **by what is being said** (agent-reasoned, same spirit as the camera choice), within `[natural-wide, MAX_HEAD]` where natural-wide = `head_h/2160` (the full-frame ratio — can't go wider than the source) and `MAX_HEAD` = 0.70 (CEO-decided, above). Wider (smaller head) for setup / new questions / lists / transitions / outro; tighter (toward `MAX_HEAD`) for emphasis / key claims / punchlines / personal beats. The CLOSE cam sits near its natural, pushing up only on the strongest beats; the WIDE cam carries the big range. **Never put both cameras at max** — vary.
...
- **VALID-CROP RULE SET (a crop must satisfy ALL, CEO 2026-06-21):** `eyeline ≤ 40%` AND `head ≤ MAX_HEAD (0.70)` AND `headroom ≥ HEADROOM_FLOOR (7%)` AND `crop ≥ 2496×1404` (1.3×FHD) AND `X-centered (face_cx ≈ 50%)`. If a level can't satisfy all, go wider (or switch camera); never ship a crop that violates any.
- **VERIFY (per run, at the BEAT START):** SAM3 on the actual rendered crop **at the beat start** → head fully in frame (crown inside, chin inside); face **X-centered** (face_cx ≈ 50%); **beat-start eyeline ≤ 40% AND near 33%**; **headroom ≥ 7%** (crown-to-top gap); crop ≥ 2496×1404 (1.3×FHD); R ≤ MAX_HEAD (0.70). (Flag any beat whose start cannot meet the rule set; do NOT re-check or correct mid-take drift.) Log per run: head% chosen, native crop size + FHD ✓, X-centered ✓, **derived eyeline% + headroom% (Y-curve)**, content reason.
