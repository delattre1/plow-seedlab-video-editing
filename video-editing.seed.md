...
- **VERIFY (per run, at the BEAT START):** SAM3 on the actual rendered crop at START / MID / END → head fully in frame (crown inside, chin inside); face **X-centered** (face_cx ≈ 50%); **beat-start eyeline ≤ 42% AND near 33%**; **headroom ≥ 7%** (crown-to-top gap); crop ≥ 2688×1512 (1.4×FHD); R ≤ MAX_HEAD (0.70). (Flag any beat whose start cannot meet the rule set; do NOT re-check or correct mid-take drift.) Log per run: head% chosen, native crop size + FHD ✓, X-centered ✓, **derived eyeline% + headroom% (Y-curve)**, content reason.
...
- **HEAD-SAFE ANCHORS — NEVER use the raw SAM3 face box as the head (CEO 2026-06-22; folded after a real "head out the top" rejection).** The SAM3 `face` box is **forehead→chin**; the hair/skull **crown sits ABOVE it**. Derive, from `face` box (`y1`=top, `y2`=bottom, `fh=y2−y1`): `head_height = 1.25·fh`; `crown = face.y1 − 0.25·fh` (the real top of head — also stated in Step 10 L686/694 and the worked example below as `ftop − 0.30·fh`); `eyes_y = face.y1 + 0.35·fh`; `chin = face.y2`. **Headroom (≥7%) and the head cap (≤70%) are measured to the CROWN / head-height, NEVER to the face-box top** — measuring headroom to the face box leaves the crop top below the real crown and **clips the hair**. Eyeline target is **CEO-tuned 36%** (≤42% hard gate), subordinate to the 7% headroom floor.
...
- **PRE-RENDER DISK-HEADROOM CHECK + POST-MUX CLEANUP (CEO 2026-06-22).** Before the segment loop, **estimate the intermediate footprint** = `n_segments × avg_per_seg` (≈ kept-minutes × ~0.5 GB/min for 4K LT) and **abort early with a clear message** if free disk < estimate × 1.5 (headroom for the final H.264 + realign temps). After the final mux+realign verifies, **delete the per-segment ProRes + frames intermediates** (and any prior completed-job work dirs) so the next long edit starts clean.
