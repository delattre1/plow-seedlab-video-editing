...
- **X — STATIC per CAMERA-RUN: ONE fixed crop-x for the whole run, a light nudge toward the run's median face — NEVER a per-window / mid-segment re-center (CEO 2026-06-25 fold, harden #5 BUG — the CEO rejected dynamic recentering / gimbal / fluid-follow in landscape; he wants only a light STATIC nudge, no follow).** Compute ONE value per run: `cx = clamp(round_even(run_median_head_cx − crop_w/2), 0, 3840 − crop_w)`, where `run_median_head_cx` = the MEDIAN head center_x across all the run's windows (a single representative nudge), and `crop_w` is the run's stable size. **Every window in the run uses that SAME `cx` (and the same `crop_w`).** Do **NOT** recompute `cx` per window from each window's `head_cx` — that makes the frame **re-position him horizontally between windows at the same zoom** (an X jump with no zoom change), which is the **dynamic recenter the CEO HATES** (see L754/L761; boss-lessons #14 / the Flow-3 correction: Cam-A static nudge, NO centering-3/fluid-follow in landscape). If the subject drifts too far within a run for one static `cx` to keep him framed, go **WIDER for the WHOLE run** (the widest gate-passing crop) or **switch camera** at a run boundary — never pan or re-center mid-run. (Fluid head-follow remains strictly OPT-IN, Step 10b, never the default.)
...
- **Center is FIXED per run** (the run's median face) — crop SIZE/level steps between runs at speech boundaries (hard step), not a continuous pan. **This is BINDING and means BOTH axes: within one camera-run, `crop_x` AND `crop_w` are CONSTANT for every window (one rectangle for the whole run, X = run-median-face nudge per the X rule above). There is NO per-window X recompute — the only thing that changes between windows of a run is which source frames play, not the crop. Crop changes ONLY at run boundaries (camera switch or a deliberate ≥10% zoom step).**
...
- **(5) SAM3 head-safe crop — the exact math (`valid_crop`), why nothing gets decapitated.** *(SAM3-TRUTH, CEO 2026-06-24 — the legacy face-box version of this example, which guessed `crown = ftop − 0.30·fh` and gated on face-span heuristics, is superseded by the SAM3-TRUTH anchors + the VALID-CROP RULE SET at L704.)* Per window,
  ...
  the head, clamp the top so the crown stays in. **X is per-RUN, not per-window** (harden #5 fix): `cx = clamp(run_median_head_cx − cw/2, 0, 3840−cw)` using the run's MEDIAN head center_x (one value for the whole run), held constant across the run's windows — NOT each window's own `head.center_x` (that would re-position him horizontally at the same zoom = the rejected dynamic recenter).
...
- **STATIC X PER RUN — `crop_x` CONSTANT within each camera-run (CEO 2026-06-25, harden #5 BUG gate):** walk the EDL; for every pair of **consecutive SAME-camera windows within a run** (no camera switch, no deliberate ≥10% zoom step between them), `crop_x` must be **IDENTICAL** (Δ = 0 px). **FAIL on any per-window X change inside a run** — that is the dynamic recenter the CEO rejected (the frame slides him horizontally at the same zoom). `crop_x` may change ONLY at a run boundary (camera switch) or a deliberate zoom step. (Confirm `crop_w` is likewise per-run-constant per the rule below.)
