...
- **HARD RULE 5 — EVERY SEGMENT MUST CROP ≥ 10% IN; NO full-frame / no-crop segment EVER (CEO 2026-06-25, harden #5 — the real bug, corrected understanding).** Every shipped shot/segment must be a **real punch-in of at least 10% from the full frame**: `crop_w ≤ 0.90·3840 (= 3456)` AND `crop_h ≤ 0.90·2160 (= 1944)`. **A full-frame (or near-full-frame, < 10% in) segment is BANNED** — that is exactly the bug the CEO caught: a segment with effectively **no crop that ONLY REPOSITIONS** (an X slide with no visible zoom) reads as broken. **REPOSITION IS NOT THE BUG — a reposition that accompanies a real crop change is CORRECT and EXPECTED**: the same camera held at different crop LEVELS (crop1→crop2→crop3) repositions at each crop change; that is intended editorial framing. The bug is *reposition with NO crop*. So the framing band is **`[10%-in … MAX_HEAD/res-floor]`** — never wider than a 10% punch-in, never tighter than HARD RULE 1/2. If a beat cannot satisfy all gates (eyeline ≤ 42, head ≤ 70, headroom ≥ 7, res-floor 2688×1512) at a **≥ 10% crop** on one camera, **switch the beat to the other camera** — NEVER ship it full-frame. (A "run" ends at the next CROP CHANGE — not necessarily a camera switch; crop size AND X are recomputed together at each crop change and held within the segment.)
...
- **Y — SHOT-SIZE-DEPENDENT placement by the AGGRESSIVE headroom→eyeline CURVE (CEO-picked 2026-06-21). NOT face-centered in Y, NOT a hardcoded eyes number — DERIVED per run from SAM3.** The vertical placement changes with shot size (cinematography convention): at WIDE the face is **headroom-anchored** (generous space above the crown); as the shot tightens, the **headroom shrinks and the eyeline rises toward the upper third (~33%)** — **bounded by `HEADROOM_FLOOR = 7%`**: the eyeline only rises as far as it can while keeping ≥ 7% headroom above the crown, so the crown never kisses the top / leaves frame on natural head movement. *(Previously the curve let the crown reach ~0% headroom at the tight end; the 7% `HEADROOM_FLOOR` now supersedes that.)*
  ...
  - **HARD GATE — eyeline ≤ 42%, evaluated (CEO 2026-06-22, relaxed from 40%), at the BEAT START, for ALL move types with NO exemption (CEO 2026-06-21). The gate applies to every move, RESETS AND FULL-FRAME INCLUDED — there is NO untouched-original exemption.** Any crop whose **beat-start eyeline exceeds 42% is NOT a usable framing**: REJECT it and fall back to a level that keeps the (beat-start) eyeline ≤ 42%.
    - **A RESET / wide / "drop to full-frame" move resolves to the WIDEST framing that still keeps eyeline ≤ 42% — NOT a raw full-frame when full-frame exceeds 42%.** Compute the widest gate-passing crop directly: `crop_h_reset = min(2160, (2160 − eyes_y) / (1 − EYELINE_GATE))` where `EYELINE_GATE = 0.42` (so the denominator is `0.58` — it is `1 − the eyeline gate`, NOT a magic number; if the gate changes, this tracks it) — the largest crop whose top-of-frame placement `cy = 2160 − crop_h` still lands the eyeline at exactly the gate (42%); `crop_w = crop_h · 16/9`, X-centered. **THE RESET IS NEVER FULL-FRAME (HARD RULE 5): cap the widest reset at a ≥ 10% punch-in — `crop_w = min(crop_w, 0.90·3840 = 3456)` (and the matching `crop_h`).** Even a high-sitting / small-head wide camera whose full-frame would pass the eyeline gate is cropped to **at least 10% in** — never raw full-frame (a full-frame segment that repositions is the bug). A low-sitting cam-A reset is a slightly-tighter wide crop as before; the only change is the hard 10%-in ceiling on the WIDE end so no segment is ever full-frame.
...
- **RESET-to-wide** — instant jump back to the **widest gate-passing level** (use on a **new question / topic / the outro**). NOTE: the reset is **still subject to the eyeline ≤ 42% gate AND to HARD RULE 5 (≥ 10% punch-in)** — it is the widest framing that keeps eyeline ≤ 42% **and is still cropped ≥ 10% in (`crop_w ≤ 3456`)**, NEVER a raw full-frame. On any camera a reset is a wide CROP, never the untouched full frame.
...
- **EVERY SEGMENT CROPS ≥ 10% — NO full-frame / no-crop segment (Step-10 HARD RULE 5, CEO 2026-06-25 — kills the no-crop-just-reposition bug):** walk the EDL; for **EVERY** shipped segment confirm a real punch-in of **≥ 10% from full frame** — `crop_w ≤ 0.90·3840 (3456)` AND `crop_h ≤ 0.90·2160 (1944)`. **FAIL the run if ANY segment is full-frame or < 10% cropped** (`crop_w > 3456` or `crop_h > 1944`). This is the gate that kills the exact bug the CEO saw — a segment left at no-crop/full-frame that only repositions. (Repositions that accompany a real ≥10% crop change are correct and are NOT flagged.)
