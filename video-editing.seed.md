...
- **RENDER_HOST ∈ {`mac` (videotoolbox), `server` (nvenc)} — a FIRST-CLASS choice, not just a fallback (C4885 fold).** The Linux/CUDA nvenc path (see PLATFORM below) is not merely the harden-test substrate; it is a production-valid render LOCATION. **Default `server` when the Mac is a shared / streaming box.** MEASURED REASON: on the Mac the encode is HW (~1 CPU core) but the M-series **media engine is SHARED with OBS** — when OBS is streaming, the HW encode QUEUES behind it and a mux CRAWLS (~35 min for a 31-min edit). "Off the Mac" is about MEDIA-ENGINE contention, NOT CPU — and it is DISTINCT from the LOCAL-ONLY disk rule below (a server render still writes the server's LOCAL disk = compliant). OPERATIONAL RECIPE (a contract to derive the script from, NOT to paste): scp the source + EDL/framing JSON + render script to the server; render there as batched **concat-FILTER → `hevc_nvenc -rc vbr -cq 19 -b:v 50M -pix_fmt p010le -tag:v hvc1`** (≈ 20 spans/batch → a per-batch mp4); the **FINAL assembly is a concat-FILTER of the batch segments, NOT `-c copy`** (concat-FILTER re-decodes, so batch→final double-nvenc at cq19/50M is fine AND it avoids the green-frame joins that `-c copy` of separately-encoded nvenc HEVC produces); scp the output back; clean the server scratch. **NB: macOS rsync 2.6.9 lacks `--info=progress2` → use `scp`.** (C4885: ≈ 10-min full render on the server GPU, zero Mac load.) Everything in PLATFORM / per-input decode / encode below still applies per host.
